<?php
namespace Luminous\Installer\Http\Controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Str;
use Illuminate\Support\Facades\DB;
use Luminous\Installer\Events\EnvironmentSaved;
use Luminous\Installer\Http\Helpers\DatabaseManager;
...
class InstallController extends Controller
{
    ...
    public function preInstall()
    {
        ...
        return view('luminous::pre_install', compact('route_value', 'resource_value', 'public_value', 'storage_value', 'env_value'));
    }

    public function serverValidation(){
        if($this->phpversion() > 8.0 && $this->mysqli() == 1 && $this->curl_version() == 1 && $this->allow_url_fopen() == 1 && $this->openssl() == 1 && $this->pdo() == 1 && $this->bcmath() == 1 && $this->ctype() == 1 && $this->fileinfo() == 1 && $this->mbstring() == 1 && $this->tokenizer() == 1 && $this->xml() == 1 && $this->json() == 1){
            session()->put('validated', 'Yes');
            return redirect()->route('Luminous::config');
        }
    ...
    public function final(Request $request)
    {
        ...
        return redirect()->route('Luminous::database')
    ...
    public function database()
    {
        ...
        if($response['status'] = 'success') {
            ...
            if (! file_exists($installedLogFile)) {
                $message = trans('Luminous Installer successfully INSTALLED on ').$dateStamp."\n";
